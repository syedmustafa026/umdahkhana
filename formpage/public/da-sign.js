...
const onResturantSignup = ()=>{
  ...
  .then((userCredential) => {
    ...
    .then((userCredential)=>{
        swal('Resturant registered')
        location.href = "da-admin.html"
    })
    .catch((error)=>{
...
logins.addEventListener("click", onResturantLogin, false)
function onResturantLogin(e) {
    e.preventDefault()
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
          ...
          firebase.database().ref(`Resturants/${Ruid}/Food_Items`).on('child_added', (data)=>{
            ...
            var itemCode = `<div class="menu" id="menu">
            <h4>Browse Menu</h4>
            <h2>YOUR MENU</h2>
        </div>
        <div class="menucards">

            <div class="menucard2 menu1">
                <div class="food1">
                    <img src="images/food2.png" alt="" height="200" width="300">
                </div>
                <h5>${ItemData.ItemName}</h5>
                <h6>$100 h6>
            </div>
        </div>`
